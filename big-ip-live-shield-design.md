...
- Shields default to **`cold`/`warm` `path_class` hook points an attacker cannot pump** (§11 — `path_class` is structure ∧ adversarial reachability); **hot-path hooks are permitted under a measured perf budget + explicit sign-off** (§11), not banned.
...
Embed the userspace eBPF VM and ship one shield against **a designed-in call site on a cold TMM path** — the worked example in §14. Implement all three modes, signing, hit evidence to SIEM, and version-based auto-retirement.
