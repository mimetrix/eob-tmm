...
| [`explainers/`](explainers/) | Visual explainers (HTML), one job each. [`programmable-dataplane-engine-v2.html`](explainers/programmable-dataplane-engine-v2.html) — **the engine**: the generic verified-eBPF utility embedded in TMM (runtime · programmable · provably safe); [`cve-mitigation.html`](explainers/cve-mitigation.html) — a plain-language explainer of data-plane CVE mitigation (the shield, one application of the engine); [`data-intelligence.html`](explainers/data-intelligence.html) — the data-plane-intelligence architecture (mechanism, API-discovery MVP reference design, the learn tier) |
