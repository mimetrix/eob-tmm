...
optimisation selection alone (it was `:=  -O2 …`, which replaced it and silently forced a
VADC trial build from `-Os` to `-O2`; corrected 2026-08-18).
...
| [`cve-survey-bnk.md`](cve-survey-bnk.md) | **Read-only Bugzilla survey, 2026-08-18.** BNK tracks **3,068** CVEs (an earlier draft said 39 --- a truncated query), and they are overwhelmingly dependency CVEs in other containers. §1.1 corrects the claim that they are unreachable *in principle*: 529 (17%) are ordinary C libraries and displacement is built for exactly that. §1.2 gives the objection that actually settles it, and it is not technical --- a shield does not change a package version, so the scanner still reports the CVE. Dependency-CVE pain is audit pain, and only an upgrade clears an audit |
...
| [`vm-capability-inventory.md`](vm-capability-inventory.md) | **The flat inventory, every axis, measured from the code.** Hook types, ctx shapes and the 96-byte ceiling (measured, not folklore), maps (hash only, 4 x 256, key 16 / value 32), helpers (**three**), verdicts, egress, admission gates, execution. Ends with the ranked extensions --- and the finding that a **clock** plus a **program-controlled emit** helper together retire the parked timer hook, for less work than the timer |
| [`ebpf-requests-capability-map.md`](ebpf-requests-capability-map.md) | **Three customer requests, answered against the code.** Attack-surface reduction (partial --- we suppress a FUNCTION, not a service; the port still listens), observability (**split**: active threats yes and uniquely, because a kernel agent cannot see TMM's decisions at all --- vulnerabilities no, because a shield does not change a package version), and defence in depth (**strongest** --- MONITOR mode already IS 'make the customer aware first'). Ends with the six gaps that block all three, and a what-to-say / what-not-to-say list |
| [`tmm-bpf-engine-architect-brief.md`](tmm-bpf-engine-architect-brief.md) | **For a review by senior TMM architects.** The mechanism in one page, twelve results already achieved on a live TMM with the evidence named, the eleven limits this audience asks about (per-call cost unmeasured, no exit probes, no signing, JIT skips the bounds callback), ten ranked extensions with cost and risk, and the six questions the room will ask --- answered |
...
| [`GROUND_TRUTH.md`](GROUND_TRUTH.md) | **Every claim about our own system, with its evidence tier** — MEASURED / SHIPPED-UNVALIDATED / ROADMAP / IDEA / FALSIFIED — and separately whether it was witnessed by us, by the kernel, or by an independent tool. A counter our own code incremented is weaker evidence than something `/proc` or `tcpdump` saw, and conflating the two is how "the hook fired" becomes "the hook works" |
| [`SOURCES.md`](SOURCES.md) | **No cached file, no claim.** Every external statement cites a file in [`evidence/cache/`](evidence/cache/) with a SHA-256. This environment has no outbound network (verified, not assumed), so remote sources are recorded `NOT_RETRIEVED` with the reason rather than paraphrased — including one CVE identifier that does not appear to be a real advisory |
| [`CONTESTED-PREMISES.md`](CONTESTED-PREMISES.md) | **Our own claims, attacked, kept rather than tidied.** Nine entries, each naming the artifact that killed it — including a slot-number assumption that was quietly a wild read on the data path, and one fix that was overturned by measurement within the same session |
| [`02-RESEARCH-PARAMETERS.md`](02-RESEARCH-PARAMETERS.md) | **Falsifier-first.** Five open questions, each pre-registered with what result would retire it, so an answer cannot be reinterpreted after the fact. Three already retired, with the killing artifact named |
| [`prototype/watchpoint/`](prototype/watchpoint/) | **Hardware watchpoints, measured rather than argued.** Two standalone programs, no TMM involved. Findings: the signal-handler objection is avoidable — `perf_event_open` breakpoints sample into a ring another thread drains, no handler in the watched thread; the privilege ask is `CAP_SYS_ADMIN`, because `CAP_PERFMON` is refused at `perf_event_paranoid=4`; the concurrency ceiling is exactly 4 (`ENOSPC` on the fifth); and a hit costs 501 ns on aarch64 against 4,755 ns on the x86 KVM guest, versus ≤ 11 ns for a pad hook. Confirms the frequency-vs-reach split with arithmetic instead of assertion |
| [`cve-selftest.md`](cve-selftest.md) | **Simulating the NULL-dereference CVE.** What is real (the signed program, the VM, the TMM process, the dereference and its fatality) and what is synthesised (the ctx, the condition, the arrival) — with both measured arms and the reason the boundary sits at the ctx rather than deeper |
| [`probe-a-function.md`](probe-a-function.md) | **The procedure, as the commands you run.** Going from a QUESTION to a running probe on a stripped binary in a live pod, without TMM's source and without a rebuild: find candidates in the signature index, check armability in the hook index, **measure reachability** (the step that gets skipped — `ip4_reject` is armable and fires zero times), generate, verify with PREVAIL's defaults overridden, install over the socket, arm by name, drain-trigger-drain, decode, disarm and check the bytes went back. Every command verified by running it. Ends with what the procedure cannot do — the socket authenticates the program but not the caller, four map slots per process, dropped fields, unmeasured per-call cost |
| [`build-pipeline.md`](build-pipeline.md) | **The generated artifacts, and why nothing has to be rebuilt.** The three per-build files (hook index, full hook map, signature index), why the order is forced (the index must come from the *packaged* binary, which only exists after `make container`, so it is a layer on top), which machine each stage runs on and why compile-verify is deliberately split from build, measured costs (**2m9s** to index every signature once, **0.10s** per lookup after --- against **1m54s** for a single uncached DWARF walk), every build-id gate and the failure it caught, and the six things the pipeline still does not do |
...
| [`explainers/README.md`](explainers/README.md) | **The reading map** — which artifact answers which question, the intended order, and what to hand a skeptic first. Start here when circulating. Each page carries a dated accuracy note in its hero; [`DOC-STATUS.md`](DOC-STATUS.md) classifies all four as **PRESENTED** and lists what each note now concedes |
