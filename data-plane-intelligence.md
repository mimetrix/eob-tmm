...
>
> The ask currently under review is a **feasibility phase**: measure the hook cost, stand up the verifier and VM path, and prove one shield end to end. **Nothing on this page is being requested, funded, or scheduled by that ask** — no fleet model, no federated training, no data product, no external feed. It is written down for two narrow reasons: so the hook and `ctx` design is not accidentally foreclosed by decisions made for the shield alone, and so the data-governance questions are visible early rather than discovered late.
>
...
>
> Everything involving an SLM, a fleet foundation model, federated training, or any external data offering is a **later, separately-authorized decision** that would need its own data-governance and legal review before it is even a proposal. Read it as a map of what the hooks could reach, not as a plan to reach it.

---

## 1. The vantage, stated honestly

Every observer of enterprise traffic is structurally blind somewhere. **So is the full proxy** — it is simply blind in fewer places, and in less damaging ones. Claiming otherwise is the fastest way to lose the room, because the bounds are well known to anyone who has run BIG-IP:
...
| **The full proxy (TMM)** | **real and enumerable:** flows accelerated in **hardware** (ePVA / FPGA / TurboFlex) never enter TMM software at all; **FastL4** paths bypass full-proxy handling; **SSL pass-through** and **client-SSL-only** virtuals are never decrypted end to end; **non-terminated UDP / QUIC** is not in protocol context; and the view is **per box and per TMM instance** — CMP/DAG decides which instance sees a given flow. Where a virtual *does* fully terminate TLS, it sees both sides of that flow, decrypted, in protocol context, at line rate. |

The defensible claim is narrower than "sees everything," and it survives contact with a TMM engineer: **for traffic a virtual actually terminates, this is the least-blind of the four vantages** — the only one that is simultaneously post-decrypt, in protocol context, and at line rate. That is still a strong position; it is just a bounded one.
...
2. **You could not compute on it in place.** Not for want of any inline surface — iRules and WASM both run logic in the data path today (`embedded-ebpf-substrate.md` §1). But they are *traffic-logic* surfaces with **no verified bound on what they may read or how long they may run**, which is precisely why nobody points them at bulk analytics over decrypted payload: unbounded code inline on cleartext application data is the crash-and-leak risk no one will accept.

**The substrate opens the second lock, and makes the first one navigable.** A bounded in-process program computes on the data *where it lives*, and what leaves the box is the distilled result — a small feature vector, a sketch, an inventory, a scoped report — never the payload.

**Now be precise about what actually holds that line, because it is not a proof.** PREVAIL proves **memory safety over the regions the host declares** (and, only when `--termination` is passed, halting). It proves *nothing* about the **semantics** of what a program returns. A verified program's 64-bit return value can carry eight bytes of a card number, and the verifier has no opinion whatsoever about that. "Data-minimization by proof" is a claim that will be corrected in the first review it reaches, deservedly, and the correction would cost more credibility than the phrase ever bought. Three concrete mechanisms do the work instead — defense in depth, not one guarantee:
...
An **F5 Traffic model** trained federated across the fleet — features and gradients move, payloads stay on the box — would be trained on **post-decrypt, in-protocol-context behavior from the enterprise-application tier**, a distribution no wire-side or log-side vendor can source. That is the bounded version of the claim, and it is the one to make. "The model that has seen more application-layer behavior than anything on earth" is not defensible against hyperscaler CDNs and WAFs, and should not be said. **All of this is annex material**: federated training over customer traffic is a separate decision with its own governance review — see the note at the top of this page.
...
**Tier 1 — on-box, no model required (prove the thesis cheaply).** This is the tier that **survives independently of everything else on this page**: no model, no fleet, no federated training, no data-governance escalation. If the rest of this annex is never pursued, Tier 1 is still worth building as a second use case for the same hooks.

- **API discovery & schema drift** — live inventory of endpoints, schemas, and sensitive-field exposure, computed in-situ; only the inventory exports. *An existing product category, fed for free — the recommended MVP (§6), and the only item here that is a candidate for near-term work.*
...
This item is also the one piece of the annex worth building **on its own merits**, independent of whether any model story is ever pursued. Everything after it in this build order is a later, separately-authorized decision.

## 6. Reference architecture — the API-discovery MVP

**Why this one first.** API discovery, schema-drift, and sensitive-data-exposure mapping is the lowest-risk, highest-leverage entry point: it's an existing security market, needs **no model** (deterministic aggregation plus light classification), runs entirely in **observe mode** (no enforcement risk), and it exercises the whole *bounded-extractor plus host-owned-sink* mechanic end to end.
...
|---|---|---|
| **Sense** | uBPF + PREVAIL (VM + verifier), clang/LLVM + libbpf/BTF (compile, typed CO-RE), Apache DataSketches / t-digest (HLL, count-min, quantiles), Feast (feature store) | the **verified in-situ extractor** + `tmmtrace` DSL, the signed TMM hook-point map + `ctx`/BTF, and the **host-owned schema-checked one-way sink** (§2 mechanism 2) |
...
Because the `ctx` is minimized by the host, egress is host-owned and schema-checked, and extractors are signed and reviewed (§2's three mechanisms), the privacy posture *strengthens* as the data use grows. "F5 reads your decrypted traffic" becomes something narrower and checkable:
