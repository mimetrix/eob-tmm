...
| change | scale |
|---|---|
| **New files added into `src/base/` (± the vulnerable-SSL overlay)** | **substrate-only: 36 files, ~8,400 lines** (34 `src/base/` + 2 whitelist config), re-measured 2026-09-05. **With the vulnerable-SSL demo overlay** (`src/modules/hudfilter/ssl/` + `ssl.c` revert): the older count was 46 files, 7,780 lines. Both are one `git status --porcelain src/` — on different tree states |
