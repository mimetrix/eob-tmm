...
### 1.1 Why this fits TMM, when DPDK and VPP answered differently

The obvious challenge is: *if embedded verified bytecode is such a good idea, why don't the two most successful userspace data planes run on it?* The honest answer is not that they tried it and rejected it — **it is that they were solving a different problem**, and five architectural differences all point the same way.

First, get the record straight, because a reviewer will check it. **VPP never evaluated bytecode for dataplane logic at all**: its BPF surface is the `bpf_trace_filter` plugin, a BPF interpreter that selects which packets to *trace*, composed with the classifier filter — observability, not forwarding or enforcement. **DPDK did build one and still ships it**: `librte_bpf` has been mainline since 18.05, with its own in-tree VM, JIT and validator. It is fair to call it low-adoption — an experimental API years on, attachable only via software RX/TX callbacks, never DPDK's headline programmability story — but it is not a corpse, and it recently gained a debuggable **abstract-interpretation** validator, i.e. it is converging on PREVAIL's technique rather than retreating from verification.

What both projects *did* choose for dataplane extension is **native-code plugins**: VPP graph nodes, DPDK applications and libraries. That is the right answer to their question and the wrong answer to ours:

- **Toolkit vs. shipped product.** DPDK is a library consumed by applications; VPP is a stack extended by plugins. In both, *the third party's code is the data plane* — so there is no vendor-owned dataplane in which to place designed-in hook points, and no single build whose debug info could yield a signed hook map. F5 owns TMM end to end, which is exactly what makes designed-in hooks, patchable function entries at named symbols, and a per-build signed hook map available to us and unavailable to them.
- **Who carries the risk.** A VPP plugin or DPDK application author is the operator, or a vendor building their own product; they accept that a bad plugin takes the process down because it is *their* process and their choice. F5 shipping a mitigation into a customer's production appliance cannot accept "it might crash the data plane." The safety proof is a requirement here and a nicety there — which is why a verifier earns its keep in our setting and looks like overhead in theirs.
- **What the alternative costs.** For a DPDK app or a VPP plugin, "just rebuild and redeploy" is cheap and routine. For a certified appliance inside a customer's change-controlled maintenance window it is weeks to quarters. **The value of dynamic change is proportional to the cost of a rebuild**, and that cost is an order of magnitude higher for TMM than for either of them.
- **Execution model — the sharp one.** eBPF's calling convention takes **one `ctx`, once**; it cannot express "here is a vector of 256 packets." VPP's entire performance premise is a graph node seeing the whole vector so it can quad-loop and prefetch across it, so a per-packet bytecode callback inside a node forfeits the architecture's reason for being. **TMM is the favorable case**: run-to-completion and core-pinned, processing per-flow and per-packet through a stack rather than as a vector through a graph, so a per-invocation hook fits the existing control flow instead of fighting it. (Where TMM *does* batch — burst receive on hot paths — DPDK already shows the answer: `rte_bpf_exec_burst()`. See [`engine-hard-problems.md`](engine-hard-problems.md) §5.)
- **What is being changed, and by whom.** Their extensibility question is "how does a third party add functionality?" Ours is "how does the vendor change the behaviour of an already-shipped, closed data plane, between releases, provably safely?" Native plugins answer the first and cannot answer the second.

Stated as a single line: **DPDK and VPP needed extensibility and bought it with native code, accepting the crash risk. We need vendor-deliverable change under proof, and the verifier is precisely what native code cannot give us.** That is the trade this substrate proposes to change — not a claim that the other projects got it wrong for their own purposes.
