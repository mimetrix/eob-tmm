...
**The support for that argument: none of the three oracles is implemented in this repo, and the pipeline is proposed, not demonstrated.** What can be said is narrower: two of the three gates are *ordinary* engineering rather than research — the verifier gate is an existing tool (PREVAIL) invoked in a build step, and exploit replay is a test harness against a proof-of-concept (PoC) exploit — while the third, a low-false-positive judgment over a legitimate-traffic corpus, needs a corpus that does not exist here. The section above is a feasibility argument and reads as one. See `explainers/cve-shield-walkthrough.html` (the worked CVE example, end to end) and `explainers/cve-mitigation.html` (the plain-language shield explainer).
