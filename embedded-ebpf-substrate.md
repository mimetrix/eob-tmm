...
| **Cheap** — JIT'd (just-in-time compiled to native code); an empty hook = one branch at a designed-in call site, and a nop pad at an unarmed function boundary | usable on cold *and* (under a measured budget) hot paths. Both the idle cost of the pads and the cost of an armed hook at rate are **unmeasured** (`engine-hard-problems.md` §1.1) |
