 ...
 *
 * D is the one the whole shadow-stack-keyed-by-SP design exists for. Build:
 *   gcc -O2 -fcf-protection=branch -o check_fexit \
 *       check_fexit.c ls_fexit.c ls_fexit_x86_64.S fexit_victims.S
 */
...
main(void)
{
    ...
    printf("    ret=%#llx exits=%llu reclaimed=%llu desync=%llu\n",
           (unsigned long long)r, (unsigned long long)g_fexit_exits,
           (unsigned long long)g_fexit_reclaimed, (unsigned long long)g_fexit_desync);
    CHECK(r == 0x1EAF, "A caller receives the real return value (stub transparent)");
    CHECK(g_fexit_exits == 1 && g_fexit_log_n == 1, "A exactly one exit observed");
    CHECK(g_fexit_log[0].retval == 0x1EAF, "A exit program sees the return value");
    CHECK(g_fexit_log[0].args[0] == 0xA1 && g_fexit_log[0].args[4] == 0xE5,
          "A entry arguments captured and carried to the exit");
    CHECK(g_fexit_reclaimed == 0 && g_fexit_desync == 0, "A no reclaim, no desync");

    puts("B --- nested: xv_mid calls xv_leaf, both armed");
    ls_fexit_reset();
    r = xv_mid(0x11, 0x22, 0x33, 0x44, 0x55);
    printf("    ret=%#llx exits=%llu\n", (unsigned long long)r,
           (unsigned long long)g_fexit_exits);
    CHECK(r == 0x1FAF, "B nested return propagates (mid = leaf + 0x100)");
    CHECK(g_fexit_exits == 2, "B two exits");
    CHECK(g_fexit_log[0].seq == 2 && g_fexit_log[0].retval == 0x1EAF,
          "B inner (leaf) exits FIRST --- LIFO");
    CHECK(g_fexit_log[1].seq == 1 && g_fexit_log[1].retval == 0x1FAF,
          "B outer (mid) exits second");
    CHECK(g_fexit_desync == 0, "B no desync");

    puts("C --- recursion: xv_rec(7), every level armed");
    ls_fexit_reset();
    r = xv_rec(7);
    printf("    ret=%llu exits=%llu reclaimed=%llu\n", (unsigned long long)r,
           (unsigned long long)g_fexit_exits, (unsigned long long)g_fexit_reclaimed);
    CHECK(r == 7, "C recursion returns n");
    CHECK(g_fexit_exits == 8, "C n+1 exits");
    CHECK(g_fexit_log[0].retval == 0 && g_fexit_log[0].seq == 8, "C deepest returns first");
    CHECK(g_fexit_log[7].retval == 7 && g_fexit_log[7].seq == 1, "C shallowest returns last");
    CHECK(g_fexit_reclaimed == 0 && g_fexit_desync == 0, "C clean nesting, no reclaim");
    ...
    CHECK(g_fexit_exits == 1, "D only the outer exit fired (inner was skipped)");
    CHECK(g_fexit_reclaimed == 1, "D the skipped inner frame was reclaimed (SP-keyed)");
    CHECK(g_fexit_log[0].retval == 0x0117, "D the outer exit sees its own return value");
    CHECK(g_fexit_desync == 0, "D reclaim resynced without desync");
