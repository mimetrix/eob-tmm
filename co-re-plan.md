...
**fexit COMPLETE end to end (2026-08-27).** #1/#2/#3 built into TMM and the loader arms an exit hook via `ls_fexit_table` (#arm-as-exit). On datkube (image `fd74821b`), a signed `fexit/` surface (`substrate/surfaces/exit_probe.bpf.c`) loaded, armed **kind=fexit** at `http_parse_client_headers`'s return, and **fired 200/200 on live traffic**, reading the parser's `ERR_*` enum return value; disarmed clean (fired frozen through 100 more requests). #4 (arm-time exclusion) and #5 (return-type admission) are now **coded and enforced** by `substrate/exit_admit.py`, wired into `bnk-build-programs.sh`: a `fexit/` surface is refused before signing unless (#5) the target's return type is rax-representable — reject float/by-value-struct/void, from the build's DWARF — and (#4) the binary imports no unwind initiator (P8 re-checked on the actual binary; if it ever fails, all exit hooks are refused pending a per-target reachability analysis). The finer per-target unwind check is also implemented (#4b): `exit_admit.py` flags a target whose `.eh_frame` FDE carries a C++ exception LSDA (a frame a stack unwind would process, which the return-address hijack could corrupt), via `llvm-dwarfdump --eh-frame` — only 38 of 72,139 FDEs have one. Tested on the build box: `http_parse_client_headers` (enum, no LSDA) ADMITTED, `ls_vm_report` (void) REFUSED (#5), a `radix_tree<...>` C++ method REFUSED (#4b LSDA); the signed `exit_probe` flowed compile→PREVAIL→admit→sign. **Covering the residual** (a future hook on a frame a linked C++ library could unwind
