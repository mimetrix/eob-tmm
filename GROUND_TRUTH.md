...
| …and is now **compiled into TMM**: the exit trampoline + shadow stack build and link into the binary | MEASURED | KERNEL | build `01680045`, `nm`: `ls_fexit_slot0`/`ls_fexit_stub`/`ls_fexit_table` + all 9 `g_ls_fexit_*` present; `diff-globals` (globals whitelist) passed `no_pgo` and `debug`. Not yet armed/run on traffic || Hardware watchpoints reach any address | MEASURED, outside TMM | KERNEL | `prototype/watchpoint/`, `perf_event_open` |
