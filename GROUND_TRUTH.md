...
| The reply still says `unverified=yes` | **FALSIFIED — a stale string** | — | verification *did* happen; that word in the reply and the "signature verified" line printed before verification are leftovers from the pre-signature loader. Cosmetic, but they are false statements in a security log and are fixed in the next build rather than left |
