...
| Arming is audited | SHIPPED-UNVALIDATED | SELF | `ls_audit.c` emits one record per control-plane operation — op, slot, hook, program hash, build range, mode ceiling, expiry, and the verdict the caller received *verbatim*. 19 assertions in `make -C substrate check` (`check-audit`) cover forged records, silent truncation, missing records for malformed input, and a verdict disagreeing with the reply. **Not yet run in a live TMM**, so F7g — that emitting a record does not wedge the loader thread — is argued and not shown |
