...
| …and PREVAIL admits it | **FALSIFIED, 2026-08-21** | INDEPENDENT | on a build box rebuilt from nothing, with PREVAIL freshly built at the pinned `06769f7b` (v0.2.5), it is **REFUSED**: `154: Upper bound must be at most 96 (valid_access(r5.offset+8, width=1) for read)` — the 96-byte context ceiling this repo measured. Refused identically without `--strict` and with `--stack-size 512`, so it is the ctx read and not the flags. The previous image carried 16 programs; this set emits 15, and the missing one is this. Whether the ctx grew past the ceiling after it last verified, or it was admitted under flags nobody recorded, is not yet established — but the claim as written does not survive a rebuild |
