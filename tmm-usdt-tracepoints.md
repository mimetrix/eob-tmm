...
- **Dark until lit.** A hook with nothing attached costs ~one predictable branch (a designed-in call site; an unarmed function boundary is a nop pad — **free at runtime**). You pay only when a program is loaded onto it, on demand. The pads are *not* free at **build** time: `-fpatchable-function-entry` inflates `.text` and carries an i-cache / i-TLB footprint that every customer pays whether or not anything is ever armed. That cost is **to be measured, not assumed** — the first experiment to run, specified in [`design-review-findings.md`](design-review-findings.md) §4 along with the three forms the mechanism can take depending on the result. **Its static half now has numbers** (BNK, x86-64): `-fpatchable-function-entry=5,0` compiles cleanly across the tree and costs **+0.476% `.text`** at 49% coverage, extrapolating to **~+0.97%** if every function were reached — but only about half the functions in the shipped binary are built by the TMM build at all, so one flag cannot reach them. Throughput, latency tail and i-cache behaviour remain unmeasured. See [`env/tmm-build-environment.md`](env/tmm-build-environment.md).
