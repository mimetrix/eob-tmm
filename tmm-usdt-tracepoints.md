...
| `tmm:hw:offload_return` | `outcome (completed/escalated/error), err, flow_key_hash, offloaded` | what came back from the accelerator, and why a flow was handed back | escalation storms, accelerator error paths — where crash bugs concentrate — and reconciling a flow's software-visible history with its accelerated span | `hot` |
