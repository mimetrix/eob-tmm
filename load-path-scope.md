...
### But B7 alone was weak evidence --- so here is the stronger test

B7 loaded the built-in shield: the same 4320 bytes already compiled into the binary and armed in
slot 0 at startup. That proves the load *path* works, not that a *different* program can be loaded,
...
```
demo_block.elf   fentry/demo_block   shield(){ return 1; }   616 bytes
demo_pass.elf    fentry/demo_pass    shield(){ return 0; }   616 bytes

1. demo_block as demo_block  -> OK loaded slot=0 mode=2              PASS
2. demo_pass  as demo_block  -> ERR load refused (identity mismatch) PASS  <-- the real evidence
3. demo_pass  as demo_pass   -> OK loaded slot=0 mode=2              PASS
4. real shield restored      -> OK loaded slot=0 mode=2              PASS
```

**Step 2 is what makes this a test rather than a demo.** If the loader were ignoring the payload and
reusing whatever was already resident, step 2 would have succeeded. It did not, so each program is
genuinely parsed and checked against its declared identity.

Note what this is *not*: an identity check is not a signature check. Nothing here authenticates
**who** produced the program --- that is item 4, still unwritten.
