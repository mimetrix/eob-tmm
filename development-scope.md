...
**The artifacts worth having as real files** — the ABI header, the schema, the checkers — live in
[`substrate/`](substrate/) and are verified by `make -C substrate check`. (A companion doc of
per-item code skeletons was retired 2026-08-26; the real substrate sources supersede those sketches.)
...
|---|---|---|---|
| **uBPF** | the VM + JIT (~150 KB) | Apache-2.0 | the calls item 3 is built on are the library's **real public API**, not invented for this proposal: `ubpf_create` / `ubpf_load_elf` / `ubpf_exec` (`vm/inc/ubpf.h:129, 458, 510` in the gitignored `ubpf/` clone), with `ubpf_compile_ex` (`:575`) as the JIT path — the *extended* mode, because the basic `ubpf_compile` (`:553`) emits a prologue that takes an unprobed 4 KiB stack frame. **This cell used to end "no runnable demonstration ships in this repo."** That is no longer true: `make -C substrate check` compiles `ls_vm.c` against the real uBPF API and `check_integrated.c` runs the whole slice — VM verdict driving an armed real function — on the bench. The load-and-run path has also been stood up in a live TMM. What the cell still does *not* claim is any of that reproducing here: `substrate/` sources are compiled into TMM in a different tree. **No longer reused (uBPF with an F5 patch; see substrate/ubpf-patches/).** The F5 fork carries a **real patch, not just a planned one**:
...
| **PREVAIL** | the static verifier | MIT **+ Apache-2.0** (the clone ships both `LICENSE` files, plus `external/{CLI11,bpf_conformance,libbtf}`; the SBOM/license scan is a Phase-1 gate per `big-ip-live-surface-design.md` §13) | stock, driven as a CLI invocation — `-q [--section <s>]` (`src/main.cpp:65, 74`). **No verify gate is demonstrated in this repo.** O3 in [`design-review-findings.md`](design-review-findings.md) records the ctx-model limit that any future demonstration has to clear, and it is a property of PREVAIL, not of any one harness |
...
| 4 | sig verify in TMM | TMM, at load | once (or reused) | small | **live, 2026-08-22** — `ls_sig.c`, Ed25519 over the 112-byte `shield_binding`, key baked into the binary at build. `bnk-test-signatures.sh` (16 assertions) passes on the cluster. Still the trust perimeter and still gates the TMA; item 9 (key lifecycle) is the unbuilt half |
| 5 | hook-map generator | build pipeline | once | tool — DWARF half conditional | **built** — `mk_hook_map.py` walks the shipped DEB pair, checks the two build ids agree, and emits 71,157 symbols with relocatability decided offline (99.7% of `.text`). Nothing is resolved by hand |
...
| 12 | audit trail | control plane | once | conventional | **live, 2026-08-20** — `ls_audit.c`; one record per control-plane op, peer pid/uid/gid from `SO_PEERCRED` (kernel-attested, not self-reported), verdict quoted verbatim from the reply. `check_audit.c` (21 assertions) + `bnk-test-audit.sh` (11) + `bnk-replay-audit.sh` (15) |
